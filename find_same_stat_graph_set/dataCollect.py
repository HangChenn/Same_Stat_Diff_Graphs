...
class DataCollection:
    ...
    def get_graph_by_gen(self, type_of_graph, distribution, vertex_num):
        if type_of_graph == "ER":
            if distribution == "P":
                graph = nx.fast_gnp_random_graph(vertex_num, self.get_ran())
            if distribution == "U":
            ...
            if distribution == "C2-": # combination of half Uniform half geometric
                coin = random.random()
                if coin < 0.5:
                    graph = nx.fast_gnp_random_graph(vertex_num, random.random())
                else:
