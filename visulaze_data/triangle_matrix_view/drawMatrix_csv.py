        ...
        for times in range(len(my_drawing_list)):
            ...
            for i in range(len(Matrix)):
                ...
                for j in range(len(Matrix)):
                    ...
                    plt.text(xMax[index]+0.07, yMax[index] - times * 0.5*yscale, "{0:.2f}".format(corr), color= self.color_f_plot[times],fontsize=12)

                    #   #######
                    if i == len(Matrix)-1 and j == len(Matrix)-1:
                        plt.text(-14*xMax[index],yscale*10-times*yscale*1.6,filename,color= self.color_f_plot[times],fontsize=20)
                    if self.graph10:
                        if times != 0:
